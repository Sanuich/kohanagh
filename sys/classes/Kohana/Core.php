...
class Core {
	...
	const VERSION  = '3.3.5.g';
	const CODENAME = 'golden hair';
	...
	public static function init(array $settings = NULL)
	{
		...
		if (self::$errors === TRUE)
		{
			...
			set_error_handler(array('\Kohana\Core', 'error_handler'));
		}
		...
		register_shutdown_function(array('\Kohana\Core', 'shutdown_handler'));
		...
		if ( ! is_writable(self::$cache_dir))
		{
			throw new Exception('Directory :dir must be writable',
				array(':dir' => Debug::path(self::$cache_dir)));
		}
	 ...
	 * @return  boolean
	 //sanuich bundles
	 classes directory sets in index file and applies only
	 for kohana classes and application folder classes
	 */
	public static function auto_load($class/*, $directory = 'classes'*/)
	{
		//sanuich changed to PSR-4	
		
		$path =  self::get_path($class);
//if(strpos($class,"404")!=false){ var_dump (self::find_class('', $path)); die();}
		if ($path =  self::find_class('', $path))
		{//if(strpos($class,"404")!=false){ var_dump($class); die();}
			// Load the class file
			require $path;
	 ...
	 * @return  string  single file path
	 //sanuich bundles
	 if function looks for class file
	 the classes subdirectory will be added to system paths only
	 */
	public static function find_file($dir, $file, $ext = NULL, $array = FALSE, $class=FALSE)
	{
		if ($ext === NULL)
		{
		...
		if ($array OR $dir === 'config' OR $dir === 'i18n' OR $dir === 'messages')
		{
			...
			{
				//if($class) $dir.=CLASSESFOLDER.DIRECTORY_SEPARATOR;
		...
		else
		{
			...
			if(empty($found) && is_file(MODPATH.$path))
			{
			// A path has been found
			$found = MODPATH.$path;
			}
			
			if(empty($found) && is_file(BUNDLESPATH.$path))
			{
			// A path has been found
			$found = BUNDLESPATH.$path;
			}
		}
	...
	public static function find_class($dir, $file, $ext = NULL, $array = FALSE)
	{
		return  self::find_file($dir, $file, $ext, $array, TRUE);
	}
	...
	/**
     * Cache variables using current cache module if enabled, if not - uses self::file_cache
     *
     *     // Set the "foo" cache
	...
	public static function message($file, $path = NULL, $default = NULL)
	{
		...
		if ( ! isset($messages[$file]))
		{
			...
			$messages[$file] = array();
			if ($files =  self::find_file('messages', $file, NULL, TRUE))
			{
				foreach ($files as $f)
				{
	...
	{
		return 'Kohana Framework '.self::VERSION.' ('.self::CODENAME.')';
	}
	
	//sanuich bundles
	//Separate namespace from rest of class name
	//Transform namespace into path or just separate and leave
	//depends on separator
	//Add prefix to last part of class name
	public static function get_path($class, $dsep=DIRECTORY_SEPARATOR)
	{
		//sanuich
		//$first_path =  self::split_class_name($class, $csep, $dsep);		
		//$sec_path =  self::split_class_name($first_path['file'],"_", $dsep);		
		//$path['path'] = $first_path['path'].$sec_path['path'];		
		//$path['file'] = $sec_path['file'];
		
		//return str_replace("\\",$dsep, str_replace("_",$dsep, $class));
		return str_replace("\\",$dsep, $class);
	}
	
	///csep: class separator
	///dsep: directory separator
	public static function split_class_name($class, $csep="_", $dsep=DIRECTORY_SEPARATOR)
	{
		$class     = ltrim($class, $csep);
		$path = '';

		if ($last_sep_pos = strripos($class, $csep))
		{
			$path		= substr($class, 0, $last_sep_pos);
			$path		= str_replace($csep, $dsep, $path).$dsep;
			$class		= substr($class, $last_sep_pos + 1);
			
		}

		return array('path'=>$path, 'file'=>$class);
	}
	/////END sanuich bundles
}