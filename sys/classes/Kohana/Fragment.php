...
class Fragment {
	...
	public static function load($name, $lifetime = NULL, $i18n = NULL)
	{
		...
		$lifetime = ($lifetime === NULL) ? self::$lifetime : (int) $lifetime;
		...
		else
		{
			...
			self::$_caches[ob_get_level()] = $cache_key;
	...
	public static function save()
	{
		...
		if (isset(Fragment::$_caches[$level]))
		{
			...
			$cache_key = self::$_caches[$level];

			// Delete the cache key, we don't need it anymore
			unset(self::$_caches[$level]);
	...
	{
		// Invalid the cache
		Kohana::cache(self::_cache_key($name, $i18n), NULL, -3600);
	}
