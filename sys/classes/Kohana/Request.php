...
class Request implements HTTP\Request {
	 ...
	 *     if (Request::initial() === self::current())
	 *          // Do something useful
	 *
	 * @return  Request
	 * @since   3.1.0
	 ...
	 * accepted, FALSE will be returned.
	 *
	 *     $encodings = self::accept_encoding();
	 *
	 ...
	 * @return  array
	 */
	public static function process(self $request, $routes = NULL)
	{
		// Load routes
	 ...
	 *
	 *     $accept = self::_parse_accept($header, $defaults);
	 *
	...
	public function __construct($uri, $client_params = array(), $allow_external = TRUE, $injected_routes = array())
	{
		...
		if ( ! $allow_external OR strpos($uri, '://') === FALSE)
		{
			...
			// Apply the client
			$this->_client = new \Kohana\Request\Client\Internal($client_params);
		}
		else
		{
			// Create a route
			...
			// Setup the client
			$this->_client = \Kohana\Request\Client\External::factory($client_params);
		}
	}
	 ...
	 * @return  self
	 */
	public function client(\Kohana\Request\Client $client = NULL)
	{
		if ($client === NULL)
			return $this->_client;
		else
