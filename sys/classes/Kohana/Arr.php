...
class Arr {
	 ...
	 *     // Returns TRUE
	 *     self::is_array(array());
	 *     self::is_array(new ArrayObject);
	 *
	 *     // Returns FALSE
	 *     self::is_array(FALSE);
	 *     self::is_array('not an array!');
	 *     self::is_array(Database::instance());
	 *
	 ...
	 *     $colors = self::path($array, array('theme', '*', 'color'));
	 *
	...
	*/
	 ...
	 *     $username = self::get($_POST, 'username');
	 *
	 *     // Get the value "sorting" from $_GET, if it exists
	 *     $sorting = self::get($_GET, 'sorting');
	 *
	 ...
	 *     self::extract($data, array('level1.level2a', 'password'));
	 *
	 ...
	 *     $array = self::map(array(array($this,'filter')), $array);
	 *
	 *     // Apply strip_tags and $this->filter to every element
	 *     $array = self::map(array('strip_tags',array($this,'filter')), $array);
	 *
	 * [!!] Because you can pass an array of callbacks, if you wish to use an array-form callback
	 * you must nest it in an additional array as above. Calling self::map(array($this,'filter'), $array)
	 * will cause an error.
