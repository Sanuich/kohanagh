...
namespace Kohana\userguide\Controller;

use Kohana\userguide\Model\Kodoc as Kodoc;
 ...
 */
class Userguide extends \Controller_Template {

	public $template = 'Kohana\\userguide\\views\\template';
	...
	public function before()
	{
		...
		else
		{
			...
			Kodoc\Markdown::$base_url  = \URL::site($this->guide->uri()).'/';
			Kodoc\Markdown::$image_url = \URL::site($this->media->uri()).'/';
		}
	...
	public function index()
	{
		...
		$this->template->content = \View::factory('Kohana\\userguide\\views\\index', array('modules' => $this->_modules()));
		$this->template->menu = \View::factory('Kohana\\userguide\\views\\menu', array('modules' => $this->_modules()));
	...
	public function error($message)
	{
		...
		$this->template->content = \View::factory('Kohana\\userguide\\views\\error',array('message' => $message));
		...
		if ($module = $this->request->param('module') AND $menu = $this->file($module.'/menu') AND Kohana::$config->load('userguide.modules.'.$module.'.enabled'))
		{
			...
			Kodoc\Markdown::$base_url  = \URL::site($this->guide->uri()).'/'.$module.'/';
			Kodoc\Markdown::$image_url = \URL::site($this->media->uri()).'/'.$module.'/';

			$this->template->menu = Kodoc\Markdown::markdown($this->_get_all_menu_markdown());
	...
	public function action_docs()
	{
		...
		Kodoc\Markdown::$show_toc = TRUE;
		$this->template->content = Kodoc\Markdown::markdown(file_get_contents($file));
		Kodoc\Markdown::$show_toc = FALSE;
	...
	public function action_api()
	{
		...
		if ( ! $class)
		{
			...
			$this->template->content = \View::factory('Kohana\\userguide\\views\\api\\toc')
		...
		else
		{
			...
			$_class = Kodoc\Clas::factory($class);
			...
			$this->template->content = \View::factory('Kohana\\userguide\\views\\api\\class')
				->set('doc', $_class)
				->set('route', $this->request->route());
		}
	...
	public function action_media()
	{
		...
		if ($file = \Kohana::find_file('Kohana/userguide/media/guide', $file, $ext))
		{
