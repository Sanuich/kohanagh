...
namespace Kohana\userguide\Model\Kodoc;
...
class Markdown extends \MarkdownExtra_Parser {
	...
	public function doBaseURL($text)
	{
		...
		return preg_replace('~(?<!!)(\[.+?\]\()(?!\w++://)(?!#)(\S*(?:\s*+".+?")?\))~', '$1'.Markdown::$base_url.'$2', $text);
	}
	...
	{
		// URLs containing "://" are left untouched
		return preg_replace('~(!\[.+?\]\()(?!\w++://)(\S*(?:\s*+".+?")?\))~', '$1'.Markdown::$image_url.'$2', $text);
	}
	...
	{
		return preg_replace_callback('/\['.\Kohana\userguide\Model\Kodoc::$regex_class_member.'\]/i', '\Kohana\userguide\Model\Kodoc::link_class_member', $text);
	}
	...
	public function doTOC($text)
	{
		...
		{
			$toc = \View::factory('Kohana\\userguide\\views\\page-toc')
				->set('array', self::$_toc)
				->render()
				;
